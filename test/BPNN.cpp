...
CBPNN::CBPNN(int layer_num, int *layer_neuron_num, float learn_rate, int learn_sample_num)
{
	...
	m_pLayers[0].Initalize(layer_neuron_num[0], 1);  
...
{
	ofstream of("Data\\study0to9 五万次 MSE 0.01.txt");
...
void CBPNN::Test(CData *pData)
{
	...
	ofstream of("Data\\测试500次single0to9.txt");
	ofstream of2("Data\\code.txt");
	if (!of.is_open())
	{
		cout << "Can't open output.txt" << endl;
	...
	for (int i = 0; i < pData->m_nSampleNum; i++)
	{
		...
		for (int j = 0; j < m_nOutputNum; j++)
		{
			...
			//	cout << "****error****";
			//cout << m_pLayers[m_nLayerNum - 1].GetNeuron(j)->m_fOutput << " ";
			of << m_pLayers[m_nLayerNum - 1].GetNeuron(j)->m_fOutput << " ";
		}
		for (int j = 0; j < m_pLayers[m_nLayerNum - 2].GetNeuronNum(); j++)
		{
			of2 << m_pLayers[m_nLayerNum - 2].GetNeuron(j)->m_fOutput << " ";
		}
		//fseek(m_pX[], 0, SEEK_END);
		cout << endl;
		of << endl;
		of2 << endl;
		cout << "end test" << endl;
